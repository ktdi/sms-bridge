...
// Alignment pattern centers per version (only non-overlapping ones)
const ALIGN_CENTERS = [
    [],           // v1
    [[18,18]],    // v2
    [[22,22]],    // v3
    [[26,26]],    // v4
    [[30,30]],    // v5
    [[34,22],[34,34],[22,34]],  // v6 - simplified
];

function isData(r,c,size){
    const ver = (size-17)/4;
    if(r===6||c===6) return false;           // timing patterns
    if(r<8&&c<8) return false;               // top-left finder
    if(r<8&&c>=size-8) return false;         // top-right finder
    if(r>=size-8&&c<8) return false;         // bottom-left finder
    if(c===8&&r<9) return false;             // left format info
    if(c===8&&r>=size-8) return false;       // bottom-left format info
    if(r===8&&c<9) return false;             // top format info
    if(r===8&&c>=size-8) return false;       // top-right format info
    if(r===7&&c<=7) return false;            // separators
...
const FIXED_TOKEN = 'SMSBRIDGE';  // hardcoded — Android app uses this same token
...
function safeSend(ws, msg) {
  ...
  } catch(e) { log(`⚠️  safeSend error: ${e.message}`); }
  return false;
}
...
function broadcast(token, msg, skip = null) {
  ...
  c.browserSockets.forEach(ws => {
    if (ws !== skip && safeSend(ws, msg)) sent++;
  });
  return sent;
}
...
const server = http.createServer((req, res) => {
  ...
  if (req.url.startsWith('/qr')) {
    ...
    // Build QR on every request so host is always correct
    const fwdHost = req.headers['x-forwarded-host'] || '';
    const fwdProto = req.headers['x-forwarded-proto'] || '';
    const host = (fwdHost || req.headers.host || '').split(',')[0].trim();
    const isLocal = host.startsWith('localhost') || host.startsWith('127.');
    const isHttps = fwdProto === 'https' || (!isLocal && !host.includes(':'));
    const wsUrl = (!isLocal && host)
      ? (isHttps ? 'wss' : 'ws') + '://' + host
      : 'ws://' + getLocalIP() + ':' + PORT;
    const qrText = wsUrl + '/' + FIXED_TOKEN;
    try {
      const svg = qrToSvg(qrText, 10);
...
wss.on('connection', (ws, req) => {
  ...
  if (clientType === 'android') {
    ...
    safeSend(ws, { type: 'connected', message: 'Server ready — send sync_data' });
    ...
    safeSend(ws, { type: 'initial_state', conversations: s.conversations, lastSync: s.lastSync, deviceName: s.deviceName, androidOnline });

    // If Android is connected, ask it to sync fresh data
    if (androidOnline) {
      log(`🔄 Requesting fresh sync from Android [${shortToken}]`);
      safeSend(c.androidSocket, { type: 'sync_request' });
    }
  ...
  switch (msg.type) {
    case 'send_sms':
      if (c.androidSocket?.readyState === WebSocket.OPEN) {
        ...
        safeSend(c.androidSocket, msg);
        broadcast(token, { type: 'sms_queued', ...msg }, ws);
        safeSend(ws, { type: 'sms_queued', messageId: msg.messageId });
        log(`📤 Send SMS to ${msg.to} queued`);
      } else {
        safeSend(ws, { type: 'error', message: 'Android is not connected' });
