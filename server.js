...
const server = http.createServer((req, res) => {
  ...
  if (req.url === '/qr' || req.url === '/qr.svg') {
    ...
    res.setHeader('Access-Control-Allow-Origin', '*');
    try {
      // Encode as compact URL: wss://host/TOKEN — fits version 3 QR (simpler = more reliable)
      const qrText = wsUrl + '/' + token;
      const svg = qrToSvg(qrText, 10);
