...
function isData(r,c,size){
    ...
    if(r<8&&c<8) return false;               // top-left finder (not separator)
    if(r<8&&c>=size-8) return false;         // top-right finder
    if(r>=size-8&&c<8) return false;         // bottom-left finder
    // Format info specific positions only (not entire row/col 8)
    if(c===8&&r<9) return false;             // left format info column
    if(c===8&&r>=size-8) return false;       // bottom-left format info
    if(r===8&&c<9) return false;             // top format info row
    if(r===8&&c>=size-8) return false;       // top-right format info
    // Separators (row/col 7 adjacent to finders)
